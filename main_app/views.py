...
def register_view(request):
    if request.method == 'POST':
        ...
        user = User.objects.create(username=username, password=password, zip_code=zip_code)
        if user:
...
def follow_user(request, username):
    if request.method == 'POST':
        try:
            ...
            return JsonResponse({'message': f'Successfully followed {username}'}, status=201)
...
def unfollow_user(request, username):
    if request.method == 'DELETE':
        try:
            ...
            Follower.objects.get(user=request.user, followed_user=user_to_unfollow).delete()
            return JsonResponse({'message': f'Successfully unfollowed {username}'}, status=200)
        except User.DoesNotExist:
            return JsonResponse({'error': f'User {username} does not exist'}, status=400)
        except Follower.DoesNotExist:
            return JsonResponse({'error': f'You are not following {username}'}, status=400)
        except Exception as e:
            return JsonResponse({'error': str(e)}, status=400)
